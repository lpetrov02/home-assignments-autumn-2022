...
def fill_around_zero(mask, i, j, r=10):
    mask[max(0, i - r):min(mask.shape[0], i + r), max(0, j - r):min(mask.shape[1], j + r)] = 0
...
def _build_impl(frame_sequence: pims.FramesSequence,
    ...
    corners_coordinates = cv2.goodFeaturesToTrack(image_0, n_corners, 0.002, 5).reshape(-1, 2)
    ...
    for frame, image_1 in enumerate(frame_sequence[1:], 1):
        ...
        old_corners = old_corners.reshape((-1, 2))
        mask_param = np.ones(image_1.shape).astype(int) * 255
        for i, cor in enumerate(old_corners):
            if status[i, 0]:
                new_ids.append(corners_ids[i])
                new_corners.append(cor)
                fill_around_zero(mask_param, int(cor[1]), int(cor[0]), r=10)
        new_ids = np.array(new_ids)
        new_corners = np.array(new_corners)

        if new_corners.shape[0] < n_corners:
            new_corners_2 = cv2.goodFeaturesToTrack(image_1, n_corners - new_corners.shape[0], 0.002, 5,
        ...
        if new_corners.shape[0] < n_corners and new_corners_2 is not None:
            ...
            corners_coordinates = new_corners
            corners_ids = new_ids
